...
def run_sha1sum(filenames: List[str], output_file: str) -> None:
    ...
    result = subprocess.run(['/usr/bin/sha1sum'] +
                            filenames, capture_output=True)
...
def create_workload(workdir: str):
    ...
    run_sha1sum(filenames, f'{path}/sha1sum.txt')

    # Create the locals file.
    with open(f'{workdir}/{LOCALS_FILENAME}', 'w') as f:
...
def verify_results(workdir: str) -> str:
    ...
    workload_sha1 = {}
    with open(f'{workdir}/{WORKLOAD_LOCAL_PATH}/sha1sum.txt', 'r') as f:
        for line in f:
            m = r.match(line.rstrip())
            if m:
                workload_sha1[os.path.basename(m.group(2))] = m.group(1)
    ...
    with open(result_filename, 'r') as f:
        for line in f:
            ...
            if m:
                ...
                if workload_hash and (workload_hash != m.group(1)):
